...
<script lang="ts">
    ...
    $effect(() => {
        // Этот эффект будет срабатывать при изменении fileUrl
        if (fileUrl) setFile();
    });
    ...
    function setFile() {
        ...
        fetch(fileUrl)
            ...
            .then((text) => {
                if (editor) {
                    // Убедимся, что редактор все еще существует
...
<div
    bind:this={editorContainerElement}
    class="ace-editor-container {className}"
>
    {#if isLoadingAce}
